app.controller('researchCtrl', function($scope, $rootScope, $routeParams, $location, $http, $sce) {
  ...
  let pages = [{
    ...
    painter:  function(db) {
               let highlighs = partitionDates(db.news, function(x) {
                  return ["Prize", "Project", "Grants", "Event", "Position","Software"].includes(x.type);
                });
                addByKeyword(highlighs, db.news, false, formatOther, function(x) {
                  return ["Prize", "Project", "Grants", "Event", "Position", "Software"].includes(x.type) ? [getDate(x), getYear(x)] : [];
                }); 
                let data = db.publications.concat(db.news); 
                let updates = partitionDates(data, function(x) {
                  return !(["Lecture Notes", "Submitted Preprints", "Prize", "Project", "Grants", "Event", "Position", "Software"].includes(x.type));
                });
                addByKeyword(updates, data, false, formatAll, function(x) {
                  return ["Lecture Notes", "Submitted Preprints", "Prize", "Project", "Grants", "Event", "Position", "Software"].includes(x.type) ? [] : [getDate(x), getYear(x)];
                });
                return [
                                    {
                    title:      "Short Bio",
  ...
  /*------------------------------
    ...
    painter:  function(db) {
                let content = partitionDates(db.news, function(x) {
                  return x.type == "Software";
                });
                addByKeyword(content, db.news, false, formatOther, function(x) {
                  return x.type != "Software" ? [] : [getDate(x), getYear(x)];
                });
                return content;
              }
  },{
